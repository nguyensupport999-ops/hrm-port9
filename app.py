...
def cc_xay_dung_file_excel_bcc(df_month, thang_v, nam_v, sunday_cols, holiday_cols):
    """
    Tạo file Excel BCC tháng TRONG BỘ NHỚ (không ghi ổ đĩa), có KHÓA DỮ LIỆU:
    - Không cho sửa NỘI DUNG/SỐ LIỆU trong các ô.
    - Vẫn cho sửa ĐỊNH DẠNG (màu, font, độ rộng cột, chiều cao dòng...).
    Trả về đối tượng BytesIO sẵn sàng đưa vào st.download_button.
    """
    from io import BytesIO
    from openpyxl import Workbook
    ...
    # ---- KHÓA DỮ LIỆU: chỉ cho sửa định dạng, KHÔNG cho sửa nội dung ----
    ws.protection.sheet = True
    ws.protection.formatCells = False     # vẫn cho đổi màu/font trong ô
    ws.protection.formatColumns = False   # vẫn cho đổi độ rộng cột
    ws.protection.formatRows = False      # vẫn cho đổi chiều cao dòng
    ws.protection.password = "honla2026"  # đổi mật khẩu tuỳ ý, hoặc để trống nếu không cần
    ...
    if 'show_chuyen_nv_form' in st.session_state and st.session_state.show_chuyen_nv_form:
        ...
        with st.form("chuyen_uv_to_nv_form", clear_on_submit=True):
            ...
            with col5:
                ...
                ty_le_dong_chuyen = st.text_input("Tỷ lệ đóng (%)", value="32", disabled=True, key="tld")
            with col6:
                _lbh_val = 0
                try:
            ...
            with st.expander("➕ THÊM NHÂN VIÊN MỚI", expanded=True, key=f"add_nv_expander_{st.session_state.add_nv_reset_ctr}"):
                ...
                with st.form(f"add_nv_{st.session_state.add_nv_reset_ctr}", clear_on_submit=True):
                    ...
                    with c5:
                        ...
                        tld = st.text_input("Tỷ lệ đóng (%)", value="32", disabled=True, key="tld")
                    with c6:
                        _lbh_val = 0
                        try:
                            _lbh_val = int(str(st.session_state.get("lbh", "0")).replace(".", "").replace(",", "").strip() or "0")
                    ...
                    try:
                        ...
                        if nd:
                            ...
                            with st.form(f"edit_nv_{nid}_{_edit_nv_ctr}", clear_on_submit=True):
                                ...
                                with col6:
                                    tldv = st.text_input("Tỷ lệ đóng (%)", value="32", disabled=True, key="tld")
                                    _lbh_e = 0
                                    try:
                                            ...
                                            if hnv:
                                                ...
                                                if ngay_loi:
                                                    ...
                                                    try:
                                                        ...
                                                        pa_val_edit = lay_ma_phuong_an(pa_edit)
                                                        ...
                                                        c_upd.execute("""UPDATE nhan_vien SET ho_ten=%s,chuc_danh_nghe=%s,ngay_sinh=%s,gioi_tinh=%s,
                                                            ...
                                                            so_luong_npt=%s, phuong_an_dieu_chinh=%s WHERE id=%s""",
                                                            (hnv, cdnv, parse_date(nsnv), gtnv, sccv, parse_date(nccv), ncv, nqnv, ttnv, (dtnv2.strip() or None) if dtnv2 else None,
                                                             ...
                                                             tkbv, nkbv, dksv, ten_don_vi_thu_huong, trinh_do_v, so_luong_npt_edit, pa_val_edit, nid))
...
- Điều 28 Luật BHXH 2014 — Mức hưởng chế độ ốm đau
            ...
            **{CHATBOT_PAYMENT['so_tien']:,.0f}đ** · {CHATBOT_PAYMENT['credit_moi']} câu hỏi tư vấn AI có viện dẫn pháp luật.\n
            Với mức thu tượng trưng nhằm tránh tình trạng Spam câu hỏi, nếu bạn không hải lòng với kết quả tư vấn - vui lòng chụp lại kết quả đó và gửi qua cho chúng tôi, chúng tôi sẽ hoàn trả phí ngay sau đó.
            """)
