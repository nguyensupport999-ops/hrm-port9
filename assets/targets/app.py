                ...
                **📄 {selected_hs['loai_ho_so']}** - {selected_hs['ten_file']}  
                ...
                if not sb:
                    ...
                    with col_download_btn:
                        dl_bytes_key = f"hoso_dl_bytes_{selected_hs['id']}"
                        if st.button("📥 TẢI HỒ SƠ", width='stretch', key=f"prep_dl_{selected_hs['id']}"):
                            try:
                                st.session_state[dl_bytes_key] = sb.storage.from_(SUPABASE_BUCKET).download(selected_hs['duong_dan_file'])
                            except Exception as e:
                                st.session_state[dl_bytes_key] = None
                                if "not_found" in str(e) or "404" in str(e):
                                    st.error(
                                        "❌ File này không còn tồn tại trong kho lưu trữ (có thể đã bị xóa hoặc "
                                        "đường dẫn lưu trong hệ thống không đúng). Vui lòng liên hệ Admin để "
                                        "kiểm tra và upload lại hồ sơ này."
                                    )
                                else:
                                    st.error(f"❌ Không thể tải file: {str(e)}")
                        if st.session_state.get(dl_bytes_key):
                            st.download_button(
                                label="⬇️ Xác nhận tải xuống",
                    ...
                    if st.session_state.get(preview_key, False):
                        try:
                            ...
                            if "not_found" in str(e) or "404" in str(e):
                                st.error(
                                    "❌ File này không còn tồn tại trong kho lưu trữ (có thể đã bị xóa hoặc "
                                    "đường dẫn lưu trong hệ thống không đúng). Vui lòng liên hệ Admin để kiểm "
                                    "tra và upload lại hồ sơ này."
                                )
                            else:
