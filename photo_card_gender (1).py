...
def render():
    ...
    st.session_state.setdefault("pcg_uploader_ver", 0)  # dùng để "làm mới" file_uploader khi bấm Thoát
    ...
    ref_file = st.file_uploader(
        ...
        key=f"pcg_ref_file_{st.session_state.pcg_uploader_ver}",
    )
    ...
    col_render_btn, col_exit_btn = st.columns([3, 1])
    with col_render_btn:
        ...
        st.markdown("</div>", unsafe_allow_html=True)
    with col_exit_btn:
        if st.button("🚪 Thoát", use_container_width=True, key="pcg_exit_btn",
                      help="Chỉ làm mới ảnh/kết quả trên màn hình, GIỮ NGUYÊN mẫu/prompt đang chọn"):
            # Chỉ reset UI (ảnh upload, kết quả, lỗi) — KHÔNG xóa pcg_selected_target,
            # nên prompt/mẫu đã chọn trước đó vẫn được giữ nguyên trong session_state.
            st.session_state.pcg_result_bytes = None
            st.session_state.pcg_error_msg = None
            st.session_state.pcg_show_manual_fallback = False
            st.session_state.pcg_uploader_ver += 1  # đổi key -> file_uploader trở về rỗng
            st.rerun()
